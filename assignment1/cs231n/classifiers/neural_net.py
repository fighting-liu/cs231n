...
class TwoLayerNet(object):
  ...
  weight matrices. ReLU nonlinearity after the first fully
  connected layer.The network uses a 
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    data_loss = -1.0 / N * np.sum(np.log(P[(range(N),y)]))
  ...
  def train(self, X, y, X_val, y_val,
    ...
    for it in xrange(num_iters):
      ...
      #########################################################################
      index = np.random.choice(range(num_train), batch_size)
      X_batch = X[index]
      y_batch = y[index]
      #########################################################################
      #                             END OF YOUR CODE                          #
